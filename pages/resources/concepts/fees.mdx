...
Forge introduces multiple pools for each token pair, each with a different swapping fee. Liquidity providers may initially create pools at three fee levels: 0.01%, 0.05%, 0.30%, and 1%. More fee levels may be added through a Governance Proposal, e.g. a 2% fee level can theoretically be added to the tier list.
